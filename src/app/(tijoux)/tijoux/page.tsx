...
function TijouxActiveSession({ onReturnToIdle }: { onReturnToIdle: () => void }) {
...
export default function TijouxPage() {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to start Dr. Tijoux session:', err);
  ...
  if (sessionCredentials) {
    return (
      <SessionWrapper
        ...
        autoStart
      >
        <AgentLifecycleView agentConfig={tijouxConfig} autoStart>
          <TijouxActiveSession
            onReturnToIdle={() => {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-[400px] space-y-6 text-center">
        <div className="space-y-3">
          <h1 className="text-5xl font-display font-semibold text-[var(--noir-text)]">Dr. Tijoux</h1>
          <p className="text-[var(--noir-text-muted)] text-xs uppercase tracking-[0.08em] font-medium">
            Clinical excavation engine
          </p>
        </div>
